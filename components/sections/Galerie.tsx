...
const Galerie = () => {
	...
	return (
		<section
			...
			data-theme='light'
			id='galerie'
			className='min-h-dvh bg-background px-6 lg:px-36 flex flex-col gap-10 relative py-10'>
			<div>
			...
			<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
				{images.map((image, index) => (
					<div
						...
						key={index}
						className='gallery-item relative group overflow-hidden rounded-lg cursor-pointer w-full h-60 lg:h-80'>
						<Image
							src={image.src}
							alt={image.alt}
							fill
						...
						<div className='absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center'>
							<span className='text-white text-sm border border-white/70 px-4 py-2 rounded-full bg-black/50'>
								Zobrazit
							</span>
						</div>
					</div>
